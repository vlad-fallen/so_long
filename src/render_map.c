...
/*   Updated: 2022/02/17 11:46:54 by mbutter          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	render_wall(t_prog_var *prog, t_vector map, t_vector pixel_map)
{
	...
	else if ((map.x == prog->map.map_size.x - 1)
		&& (map.y == prog->map.map_size.y - 1))
	...
	else if ((map.y == prog->map.map_size.y - 1) && (map.x > 0
			&& map.x < prog->map.map_size.x - 1))
...
void	selection_of_render(t_prog_var *prog, t_vector map, t_vector pixel_map)
{
	...
	{
		render_image(prog->space.empty_space, pixel_map.x, pixel_map.y, prog);
		render_image(prog->player.img_player[0], pixel_map.x,
			pixel_map.y + 6, prog);
	}
	else if (prog->map.map[map.y][map.x] == 'C')
	{
		...
		render_image(prog->space.empty_space, pixel_map.x, pixel_map.y, prog);
		render_image(prog->diamond.img_collect, pixel_map.x + 8,
			pixel_map.y + 12, prog);
	}
	else if (prog->map.map[map.y][map.x] == 'E')
	{
		render_image(prog->space.empty_space, pixel_map.x, pixel_map.y, prog);
		render_image(prog->exit.img_escape[0], pixel_map.x + 9,
			pixel_map.y + 8, prog);
	}
}

static void	set_coordinates(t_prog_var *prog, t_vector map, t_vector pixel_map)
{
	if (prog->map.map[map.y][map.x] == 'P')
	{
...
void	render_map(t_prog_var *prog)
{
	...
	while (map.y < prog->map.map_size.y)
	{
		...
		while (map.x < prog->map.map_size.x)
		{
			...
			set_coordinates(prog, map, pixel_map);
			map.x++;
			pixel_map.x += 64;
		}
		map.y++;
